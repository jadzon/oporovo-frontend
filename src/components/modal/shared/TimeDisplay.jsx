    ...
    return (
        <div className="mt-2">
            <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                <motion.div
                    className={`h-full ${timeStatus.isLive ? 'bg-blue-500' : timeStatus.timePercentage === 100 ? 'bg-green-500' : 'bg-gray-300'}`}
                    initial={{ width: 0 }}
            ...
            <div className="flex justify-between items-center mt-1.5">
                <p className={`text-sm font-medium ${timeStatus.isLive ? 'text-blue-600' : timeStatus.timePercentage === 100 ? 'text-green-600' : 'text-gray-600'}`}>
                    {timeStatus.timeText}
                </p>
                {timeStatus.isLive && (
