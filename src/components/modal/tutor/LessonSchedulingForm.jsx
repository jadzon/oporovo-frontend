...
export const LessonSchedulingForm = ({ tutor, error, setError, onFormSubmit }) => {
    ...
    useEffect(() => {
        if (startTime && endTime) {
            ...
            const calculatedPrice = timeUtils.calculatePrice(startTime, endTime, tutor?.price || 0)
            setTotalPrice(calculatedPrice)
        } else {
            setLessonDuration(0)
            setTotalPrice(0)
    ...
    const fetchMonthAvailability = async () => {
        ...
        try {
            ...
            const response = await tutorAvailabilityService.getAvailability(tutor.id, startDate, endDate)
    ...
    const handleSubmit = (e) => {
        ...
        // Start is from selected block, end depends on whether there's an adjacent block
    ...
    return (
        <div className="space-y-6 p-4">
            <div className="text-center mb-6">
                <h2 className="text-xl font-semibold text-gray-800 mb-2">Zaplanuj lekcję z {tutor?.first_name}</h2>
                <p className="text-sm text-gray-600">Wybierz termin i podaj szczegóły lekcji</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        <Card title="Wybierz termin" icon="calendar">
                        ...
                        {selectedDay && (
                            <Card title="Wybierz godziny" icon="clock">
                                ...
                                {selectedBlock && (
                                    <div className="mt-4 space-y-2">
                                        <div className="flex justify-between items-center text-sm">
                                            ...
                                            <span className="font-medium text-gray-800">{lessonDuration} minut</span>
                                        </div>
                                        {tutor?.price && (
                                            <div className="flex justify-between items-center text-sm">
                                                <span className="text-gray-600">Cena lekcji:</span>
                                                <span className="font-medium text-black">{formatUtils.formatPrice(totalPrice)}</span>
                                            </div>
                                        )}
                                    </div>
                                )}
                    ...
                    <div className="space-y-4">
                        <Card title="Szczegóły lekcji" icon="edit">
                            <div className="space-y-4">
                                <div>
                        ...
                        <button
                            ...
                            className="w-full px-4 py-2.5 bg-black hover:bg-gray-800 text-white rounded-full font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2"
                        >
